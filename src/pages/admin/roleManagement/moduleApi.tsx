...
import { GroupPermission } from '@/globalType/permission.type'
...
interface IProps {
  ...
  listPermissions: GroupPermission[] | null
...
const ModuleApi = (props: IProps) => {
  ...
  useEffect(() => {
    if (listPermissions?.length && singleRole?.id && openModal === true) {
      ...
      const userPermissions = groupByPermission(singleRole.permissionIds)
      ...
      listPermissions.forEach((x) => {
        ...
        x.permissions?.forEach((y) => {
          const temp = userPermissions.find((z) => z.model === x.model)
        ...
        // form.setFieldValue(["permissions", x.model], allCheck);
        p[x.model] = allCheck
      })
      console.log(p)
      form.setFieldsValue({
  ...
  const handleSwitchAll = (value: boolean, name: string) => {
    const child = listPermissions?.find((item) => item.model === name)
    if (child) {
  ...
  const handleSingleCheck = (value: boolean, child: string, parent: string) => {
    ...
    //check all
    const temp = listPermissions?.find((item) => item.model === parent)
    if (temp?.model) {
  ...
  const panels: CollapseProps['items'] = listPermissions?.map((item, index) => ({
    key: `${index}-parent`,
    label: <div>{item.model}</div>,
    forceRender: true,
    extra: (
      <div className='customize-form-item'>
        <ProFormSwitch
          name={['permissions', item.model]}
          fieldProps={{
            defaultChecked: false,
            onClick: (_u, e) => {
              e.stopPropagation()
            },
            onChange: (value) => handleSwitchAll(value, item.model)
          }}
        />
      </div>
    ),
    children: (
      <Row gutter={[16, 16]}>
        {item.permissions?.map((value, i: number) => (
          <Col lg={12} md={12} sm={24} key={`${i}-child-${item.model}`}>
            <Card size='small' className='flex flex-row'>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                <ProFormSwitch
                  ...
                  fieldProps={{
                    defaultChecked: false,
                    onChange: (v) => handleSingleCheck(v, value.id as string, item.model)
                  }}
                />
              </div>
